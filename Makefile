...
.PHONY: test
test:
	PYTHONPATH=$(PYTHONPATH) poetry run pytest -c pyproject.toml --cov-report=html --cov=filigree tests/
...
check-codestyle:
	poetry run darglint --verbosity 2 filigree tests
...
check-safety:
	poetry check
	poetry run bandit -ll --recursive filigree tests
